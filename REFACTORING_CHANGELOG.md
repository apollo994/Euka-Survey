...
## 2026-06-15 — Batch 11: Phase 3 — Metric config table (#31)

Closes Phase 3 #31 + architectural improvement F. The audit flagged
"10+ hardcoded references to `ass/ann/rna/lng`" duplicated across
the data layer, the bar-chart renderer, the UI filter/sort dropdowns,
and the TSV exporter. They are now derived from a single config.

### `src/metrics.py` (new)

Frozen `Metric` dataclass with the four fields each consumer needs:

- `key` — column-name suffix (`ass`/`ann`/`rna`/`lng`).
- `title` — display title.
- `color`, `side` (`"left"`/`"right"`), `overlay` (bool) — drive the
  divergent bar chart in `visualization.py`.
- `legend_label` — past-participle adjective for the legend
  (`Assembled` / `Annotated`) where it differs from `title`.
- `filter_label`, `sort_count_label`, `sort_total_label` — feed
  the `app.py` multiselect + sort dropdown.
- `tsv_count_column`, `tsv_total_column` — public TSV column names.

Plus derived helpers (`coverage_key` / `total_key` / `percent_key`
properties on `Metric`; `METRIC_KEYS` / `COVERAGE_KEYS` / `TOTAL_KEYS` /
`PERCENT_KEYS` module-level tuples).

`METRICS` is a `tuple` (not a list) — order is load-bearing for the
SQL column layout, the bar-chart zorder, and the TSV column ordering;
mutability would be a footgun.

### Consumers refactored to read from METRICS

| File | What now flows from METRICS |
|---|---|
| `src/database.py` | `_COUNT_KEYS`, `_COVERAGE_KEYS`, `_SQL_COLUMNS`, `_row_to_metadata`, `_empty_metadata` |
| `src/visualization.py` | Bar-chart `ax.barh` calls (single loop replaces four hardcoded calls); legend rows pull labels + colors via a `(side, overlay)` lookup; per-leaf count columns generated by enumerating METRICS instead of four hardcoded `_add_count_col` calls |
| `app.py` | `filter_options` and `sort_options` dicts derived from METRICS — labels and keys are byte-identical to the pre-refactor literals (verified) |
| `src/utils.py` | TSV header + row built from a single `(name, value_fn)` list so the header cannot drift from the row |

### `tests/test_metrics.py` (new)

13 sanity tests, three concerns:

1. **METRICS shape** — exactly 4 metrics; canonical keys
   `(ass, ann, rna, lng)`; unique keys; hex-triplet colors; sides split
   2/2; each side has 1 light + 1 overlay; derived key tuples
   in METRICS order; per-metric property helpers agree.
2. **Public schema** — TSV column names unique and snake_case; filter
   and sort labels unique (Streamlit selectboxes would otherwise
   silently merge duplicates).
3. **Database drift guards** — `_COUNT_KEYS`, `_COVERAGE_KEYS`, and
   `_SQL_COLUMNS` must derive from `METRICS` in matching order. Catches
   the bug where someone reorders METRICS but the SQL unpacking still
   assumes the old layout.

### Verified end-to-end

- Pre-refactor literals byte-identical against derived dicts (filter
  options, sort options, TSV header + row).
- Bar chart still produces #1f78b4 (annotations overlay) on the left
  half and #b2df8a (RNA-Seq Any base) on the right half at the
  expected x offsets — pixel sampled from the rendered PNG.
- Full suite: 83 passed, 3 skipped (was 73 / 3 — +10 metric tests).

### Why not also pull summary-card copy + URLs into METRICS

Each card in `app.py`'s "Genomic Resource Summary" section has
distinct URL templates (NCBI Datasets / Annotrieve / ENA advanced
search) and distinct help-text per resource. Pulling all that into
`Metric` would more than double the dataclass surface without
collapsing duplication — each card would still be a one-off render
block. Deferred to Phase 3 #29 (the `app.py` UI split), where the
