...
| 2 | Extract a single `NCBITaxa` module-level singleton/accessor (with thread-safe lazy init) and reuse it across `src/taxonomy.py`, `src/ete_utils.py`, `src/visualization.py`, `app.py`, and `db_builder/` | High — currently 5+ instantiations per Streamlit rerun, opens many SQLite handles, slows hot path | Low | **DONE** (2026-06-15) — `src/ete_utils.get_ncbi()` is a `threading.local()`-cached accessor; one `NCBITaxa` per worker thread for the process lifetime. All 7 prod call sites (ete_utils ×3, taxonomy ×2, visualization, precompute_aggregations) routed through it. Same-thread identity + cross-thread distinctness covered by `tests/test_ete_utils.py`. |
...
- **H4.** ✅ *(2026-06-15)* `NCBITaxa` instantiated 5+ times per rerun — `src/ete_utils.get_ncbi()` thread-local accessor closes this. Lookup `lru_cache` remains as a complementary layer; all prod call sites use the accessor.
...
18. ✅ *(2026-06-15)* Single `NCBITaxa` accessor in `src/ete_utils.py`; replace all `NCBITaxa()` calls. *(Batch 15: `get_ncbi()` with `threading.local()` cache — one instance per worker thread for the process lifetime, sidestepping ETE3's underlying sqlite3 `check_same_thread`. Replaces 7 prod call sites across `src/ete_utils.py`, `src/taxonomy.py`, `src/visualization.py`, and `db_builder/precompute_aggregations.py`. Covered by `tests/test_ete_utils.py` — same-thread identity + cross-thread distinctness.)*
